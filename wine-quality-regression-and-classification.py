...
def wine_neural_net(winedata, winetype, alpha_1, hiddenlayersize):
    ...
    clf = MLPClassifier(solver='lbfgs', alpha=alpha_1, hidden_layer_sizes=hiddenlayersize, random_state=1)
    clf.fit(X, y)
    
    y_pred_MLPC = clf.predict(X_test)
    print('F1 score: ' + str(sklearn.metrics.f1_score(y_test, y_pred_MLPC,average='weighted')))
    return sklearn.metrics.classification_report(y_test, y_pred_MLPC), sklearn.metrics.f1_score(y_test, y_pred_MLPC,average='weighted'), hiddenlayersize
...
wine_datas = [('winequality-red.csv', 'red'), ('winequality-white.csv', 'white')]

for wine in wine_datas:
    wine_data_analyzer(wine[0], wine[1])
    
    hidden_layers_test = [(5, 2), (5, 2, 2), (4, 3, 3), (6, 6, 6), (5, 2, 2, 2), (8, 6, 4, 2), (4, 4, 4), (3, 3, 3, 3), (7, 3, 3) ]
    results = []
    for hiddenlayer in hidden_layers_test:
        results.append(wine_neural_net(wine[0], wine[1], 1e-5, hiddenlayer))        
    best_result_hl = max([x[1] for x in results])
       
    for x in results:
        if x[1] == best_result_hl:
            print(wine[1] + ' wine \n' + 'Best hidden layer size is: ' + str(x[2]) + ' which achieves F1 score of: ' + str(round(x[1], 4)) + '.')
            print(x[0])